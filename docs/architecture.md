...
通用自动化默认按垂直误差精简点数，并保护边缘和全部区外点；密集点模式为显式选项。原生音高以绝对MIDI曲线表示，转换组时间与音高偏移后创建独立宿主对象，未应用时不附加到工程。跨界原生曲线与区内引导点仍拒绝；已有非零 pitchDelta 原样保留，预览常显该说明且不把控制曲线当作最终音频基频。音高偏移完整指纹参与应用和恢复校验，依赖变化后必须重新预览。两类参数使用相同的预览编号、应用/回滚/恢复流程；完整原生快照包含对象类型、次序、锚点、内部点及可读脚本元数据。
